...
const SummaryCard = ({
  ...
  trend = null
    ...
    <CardContent>
      ...
      {trend && <div className={`text-xs mt-2 flex items-center ${trend.positive ? 'text-green-600' : 'text-red-500'}`}>
    ...
    <CardContent>
      <div className="h-[200px]">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={data} cx="50%" cy="50%" innerRadius={50} outerRadius={80} fill="#8884d8" paddingAngle={2} dataKey={dataKey} nameKey={nameKey} label={({
            name,
            percent
  ...
  return <Card className="col-span-1">
      ...
      <CardContent>
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={dataWithPercentage} margin={{
            top: 20,
            right: 30,
            left: 20,
            bottom: 5
              ...
              return <g transform={`translate(${x},${y})`}>
                      ...
                      {Icon && <foreignObject x={-8} y={25} width={16} height={16}>
              ...
              <Legend />
              <Line type="monotone" dataKey="time" name="Tiempo (min)" stroke={COLORS.primary} strokeWidth={3} dot={props => {
              const {
                cx,
                cy,
                payload
              } = props;
              return <circle cx={cx} cy={cy} r={6} fill={getColor(payload.name)} stroke="white" strokeWidth={2} />;
            }} />
              <Line type="monotone" dataKey="cumulative" name="Tiempo Acumulado" stroke={COLORS.tertiary} strokeDasharray="5 5" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
...
const SalesDashboard = () => {
  ...
  return <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <SummaryCard title="Ventas Totales" value={`S/ ${totalSales.toLocaleString()}`} description="Últimos 7 días" icon={DollarSign} trend={{
        positive: true,
        value: 12.5,
        text: "vs. semana anterior"
      }} />
        <SummaryCard title="Pedidos" value={totalOrders} description="Órdenes completadas" icon={Receipt} trend={{
        positive: true,
        value: 8.2,
        text: "vs. semana anterior"
      }} />
        <SummaryCard title="Ticket Promedio" value={`S/ ${avgTicket.toFixed(2)}`} description="Por orden" icon={ChartBar} trend={{
        positive: true,
        value: 3.7,
        text: "vs. semana anterior"
      }} />
        <ServiceTimeCard avgTime={avgServiceTimePerOrder} trend={serviceTimeTrend} />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
        <PieChartComponent data={paymentMethodData} colors={[COLORS.primary, COLORS.secondary, COLORS.tertiary, COLORS.quaternary]} title="Ventas por Método de Pago" />
        <PieChartComponent data={orderTypeData} colors={[COLORS.delivery, COLORS.pickup]} title="Ventas por Tipo de Entrega" />
            ...
            <div className="relative h-40 w-40">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  name: "No convertidos",
                  value: 10200
                }]} cx="50%" cy="50%" innerRadius={55} outerRadius={70} startAngle={90} endAngle={-270} paddingAngle={0} dataKey="value">
            ...
            <div className="h-[300px]">
              ...
              orders: {
                ...
                <LineChart data={dailySalesData}>
                  ...
                  <ChartTooltip content={<ChartTooltipContent />} />
                  <Legend />
                  <Line type="monotone" dataKey="sales" name="Ventas (S/)" yAxisId="left" stroke="var(--color-sales)" strokeWidth={2} />
                  <Line type="monotone" dataKey="orders" name="Órdenes" yAxisId="right" stroke="var(--color-orders)" strokeWidth={2} />
                </LineChart>
              </ChartContainer>
            </div>
          </CardContent>
